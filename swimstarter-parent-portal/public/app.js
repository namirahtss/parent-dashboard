...
VIEWS.home = async function () {
  ...
  root.innerHTML = `
    <div class="section-label" style="margin-top:0">My child</div>
...
function renderProfilePanel(c, editing = false) {
  ...
  if (!editing) {
    p.innerHTML = `
      ...
      <div class="kv"><span class="k">Pool location</span><span class="v">${esc(c.location || "—")} <span id="pool-status"></span></span></div>
...
// Home: render announcements only, and set the child's own pool open/closed badge.
async function loadNoticesAndPool(c) {
  const box = $("#home-notices");
  if (box) box.innerHTML = `<div class="section-label">Announcements</div><div class="panel"><div class="loader">Loading…</div></div>`;
  try {
    const data = await api("/announcements");

    // pool open/closed badge for the child's location only
    const ps = $("#pool-status");
    if (ps && c && c.location) {
      const mine = (data.pool || []).find((p) => p.location && p.location.toLowerCase() === c.location.toLowerCase());
      if (mine) {
        const open = /open|yes/i.test(mine.canEnter || mine.status || "");
        ps.innerHTML = `<span class="badge ${open ? "green" : "red"}">${open ? "Open" : "Closed"}</span>`;
      }
    }

    // announcements list
    let html = `<div class="section-label">Announcements</div>`;
    const items = (data.announcements || []).slice(0, 4);
    if (!items.length) {
      html += `<div class="panel"><div class="empty">No announcements right now.</div></div>`;
    } else {
      items.forEach((a) => {
    ...
    if (box) box.innerHTML = `<div class="section-label">Announcements</div><div class="panel"><div class="empty">${esc(e.message)}</div></div>`;
  }
}
  ...
  try {
    ...
    root.innerHTML = data.children.map((c) => `
      <div class="panel pay-card">
        <h3 style="margin-top:0">${esc(c.childName)}</h3>
        <div class="pay-dates">
          <div class="pay-date"><div class="lbl">Term start</div><div class="val">${fmtDate(c.paidTermStart) || "—"}</div></div>
          <div class="pay-date"><div class="lbl">Term end</div><div class="val">${fmtDate(c.paidTermEnd) || "—"}</div></div>
        </div>
