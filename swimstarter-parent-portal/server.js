...
app.get("/api/slots", auth, async (req, res) => {
  try {
    ...
    // Parents can book any open slot, so we show ALL empty rows (any coach).
    const rows = await emptyRows({});

    // Their own default day/time is excluded from EXTRA booking (must be outside it).
    const myDays = asNameList(child.get("Lesson Day"));
    const myTimes = asNameList(child.get("Timeslot"));
    const isDefault = (day, ts) => myDays.includes(day) && myTimes.includes(ts);
    ...
    for (const r of rows) {
      ...
      if (!day || !ts) continue;
      if (scope === "book" && isDefault(day, ts)) continue; // outside default timing only
      byDay[day] = byDay[day] || {};
    ...
    res.json({ days, windowDays: BOOK_WINDOW_DAYS });
  } catch (err) {
...
const BUILD_VERSION = "2026-07-14-fix4-home-pool-payment";
